...
| 名前 | 意味 | 署名（コメント） | 引数 | 返り値 |
| --- | --- | --- | --- | --- |
| `member-posn?` | 座標が生存リストに**あるか** | `Posn ListOfPosn -> Boolean` | `cell`, `cells` | いれば `true` |
| `count-neighbors` | あるマスの**近傍生存数** | `Posn ListOfPosn -> Number` | `cell`=中心, `live`=今の生存リスト | 0〜8 の数 |
| `count-alive-in` | 座標リストのうち、生存に含まれる**個数** | `ListOfPosn ListOfPosn -> Number` | `neighbors`, `live` | 個数 |
