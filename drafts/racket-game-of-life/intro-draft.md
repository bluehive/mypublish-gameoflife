...
- **教育用言語レベル**（Beginning → Intermediate → **Advanced Student**）から本格 `#lang racket` まで段階を踏める
- **画像・アニメーション**（`2htdp/image` / `2htdp/universe`）や GUI（`racket/gui`）が揃っている
- **テスト**: ASL では `check-expect`、本格 Racket では `rackunit` も使える

「まず動く小さな式を REPL で試す → 関数にまとめる → テストする」という循環が、DrRacket だととてもやりやすいです。

本編のサンプルコードは **Advanced Student**（`#lang htdp/asl`）を第一言語とします（[mypublish-gameoflife#1](https://github.com/bluehive/mypublish-gameoflife/issues/1)）。  
理由は、`check-expect` によるテストが書きやすく、エラーメッセージが初学者にも追いやすいことです。HtDP のデザインレシピは第1〜2章相当で取り込みます（詳細は #28 と `htdp-ja-translation`）。本格 `#lang racket` は発展・対照用の位置づけです。
...
本リポジトリ周辺では、すでに純粋関数型のコアと表示を持つ実装があります（`my-racket/lifeofgame.rkt` など）。第4〜5章では、ASL 上で生存セルを `posn` のリストとして**読み解けるように書き直し**、`check-expect` とパターンを足していきます。

#### 0.4 この本の進め方（7月〜9月の位置づけ）

- **7月末までの成功定義（暫定固定）**: 次の **3本の章ドラフト** を埋める  
  1. 序章＋第1章（基礎・式・リストの入口）  
  2. 第4章（ライフゲームのルールと `check-expect` / ASL）  
  3. 第5章の骨格（描画・有名パターン）  
