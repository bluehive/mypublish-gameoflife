...
0.5 の「進め方の予告」は **どの章をどの順で読むか** の地図です。ここにある図はそれとは別で、**各章でくり返す作業の型**——howtocode / HtDP が教えるベースライン——を一枚にしたものです。章の目次ではなく、「どうやって1つの関数を設計するか」の流れです。

核になるのは次の二つです。

- **例** … 具体的な入力と、期待する結果（あとで `check-expect` になる）
- **テンプレート** … データの形に沿った関数の骨組み（`...` で空けた型紙）
...
```text
  ...
  凡例: ★ = 飛ばしがちだが、迷子を防ぐ二本柱
        例 = 「動く前に答えを書く」
        テンプレート = 「データの形がコードの形を決める」
```

0.6 の箇条書きは、上図の**部品名の一覧**です。第2章でテンプレートを本格的に書き、第3–5章で盤・規則・表示に同じ流れを載せます。いきなり完成コードを目指すのではなく、**例とテンプレートを先に置く**のが本書の意図です。
