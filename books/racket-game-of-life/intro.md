...
0.5 の「進め方の予告」は **どの章をどの順で読むか** の地図です。ここにある図はそれとは別で、**各章でくり返す作業の型**——howtocode / HtDP が教えるベースライン——を一枚にしたものです。章の詳細目次の再掲ではなく、「どうやって1つの関数を設計するか」と「それがどの章・どの付属コードに載るか」のハブです。

##### 用語の錨（短く）

| 用語 | 意味（序章での錨） |
|------|-------------------|
| **例** | 具体的な入力と、先に書いておく期待結果 |
| **check-expect** | その例を Run で確かめる形 |
| **テンプレート** | データの場合分けに沿った、空き（`...`）付きの骨組み |
| **stub** | まだ正しい計算をしない仮の本体（形だけ先に置く） |
| **署名 / 目的** | 型の約束と、日本語1文の「何をする関数か」 |

核になる二本柱は、いぜん **例** と **テンプレート** です。残り（署名・stub・Run）は、この二つを挟んで迷子にならないための手順です。
...
```text
  ...
  └───────────────────────────────────────────┘
           ...
           │ 同じ型を、章の部品に載せ替える
           ▼
  ch01  式・define・cond・リスト入口 / check-expect
    │     付属: code/ch01-basics.rkt
    ▼
  ch02  データ定義・例・テンプレ・再帰（本丸）
    │     付属: code/ch02-recursion.rkt
    ▼
  ch03  盤データ ListOfPosn（本線）
    │     付属: code/ch03-grid.rkt
    ▼
  ch04  ライフ規則をテンプレ＋例で固定
    │     付属: code/ch04-life-rules.rkt
    ▼
  ch05  目視とテストでループを閉じる
          付属: code/ch05-display.rkt
          （発展: ch05-display-isl.rkt / big-bang 系）

  凡例: ★ = 飛ばしがちだが迷子を防ぐ二本柱
        付属コードの一覧と開き方 → 付録 A
```

0.6 の箇条書きは上図の**部品名の一覧**、上の ch01–05 は**載せ替え先の章対応**（各章の #### をすべて写したものではない）です。いきなり完成コードを目指すのではなく、**例とテンプレートを先に置く**のが本書の意図です。付属 `.rkt` の中身の改変はこの節の目的ではなく、Run する場所の目印です。
