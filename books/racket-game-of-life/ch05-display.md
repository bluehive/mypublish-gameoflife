...
**コラム: イベントと big-bang（対応の見取り図）**

「時間が進んだら何かする」「今の状態を画面に出す」「キー入力で状態を変える」というのは、テキストのインタラクティブプログラムではだいたい次の対応になります。**新しい魔法ではなく、同じ考え方の別表記**です。
...
- **キーが押されたら状態を変える（入力の働き）**  
  → **`on-key`**  
  （一時停止、1世代だけ進める、など）
- **最初の状態から始める（起動の働き）**  
  → 初期の世界を渡して起動する … **`(big-bang 初期状態 …)`** の第1引数

だから 5.6 は「ゼロからイベント駆動を学ぶ」節ではなく、**big-bang の部品名とライフの対応を先に掴む見取り図**だと思ってください。実装は発展で十分です。
