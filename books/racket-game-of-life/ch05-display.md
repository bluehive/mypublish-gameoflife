...
イメージで言えば、**大きな紙の地図の上に小さな枠のカードを置いて、枠の中だけを写し取る**感じです。ライフの盤は理屈上どこまでも広がれますが、今見たいのはそのうちの長方形だけ。Scratch の「カメラでステージの一部を見る」や、スマホの地図アプリで枠を動かして一部だけ表示する、に近い感覚です。テキストで覚えるなら「**枠の左上が盤のどこか（原点）**」と「**枠が何マス横・何マス縦か（幅・高さ）**」の2点だけです。
...
署名の読み方のコツ: **矢印の右（返り値）から見ます**。`ListOfString` は「1行が1本の文字列になった、上から下への行のリスト」です。次に左（引数）を「何を渡すか」と並べます。`ListOfPosn` が生きているマス、続く4つの `Number` がビューポート（原点 x・原点 y・幅・高さ）です。「生存リスト＋のぞき窓4つ」と口に出すと、`world->rows`（世界を行へ）という名前ともつながります。
...
**（2）グライダー世代 0〜8**（ビューポート **12×10**、原点 (0,0)、初期は少し内側の `glider-0`）

各世代の前に `;; --- generation N ---` が付き、升目が続きます。周期 4 で形が戻り、だいたい右下へ進むのが目で分かります（第4章のグライダーと同じ）。出力は長いのでスクロールして見てください。

```text
;; === glider generations 0..8 (viewport 12x10, origin 0,0) ===
;; --- generation 0 ---
............
..#.........
...#........
...
中身のルール（`next-generation`）は第4章／本線 `ch05-display.rkt` と**同型の抜粋**です。本線の必須理解は BSL 側で足り、ISL+ は表示体験用です。
...
第4章のパターンテストと**同じ命名規則**（意味が分かる名前を付ける）を使いますが、**識別子の先頭に `pattern-` が付く**点に注意してください（第4章は `block` / `blinker-h`、本章は `pattern-block` / `pattern-blinker`）。また**座標の起点も違います**: 第4章の `block` は原点 (1,1) 起点、第5章の `pattern-block` は (0,0) 起点です（形は同じ）。カタログは「見た目の名前」と「コードの定数」を対応させる辞書です。
...
- `code/ch05-display-isl.rkt`（発展）— 9×9 ブロック表示 ＋ グライダー世代 0〜8（12×10）、`#lang htdp/isl+`
