...
**なぜ「いっせい」でないとダメなのか** — ルールは「**いまの盤だけ**を見て次の盤を決める」約束だからです。もし1マスずつ順番に進めると、先に変わったマスが隣の近傍数に食い込み、「誰から更新したか」で結果が変わってしまいます。例えば左から順に消していくと、右のマスは「まだ生きている左」を見てしまい、いっせい更新とは別の世界になります。**全員が同じ写真（今の盤）を見て一斉に次の写真へ進む**、というイメージで考えると分かりやすいです。

ここで「でもコードは1個ずつ計算するじゃん？」と思うかもしれません。確かに `next-generation` は生存リストを先頭から1つずつ処理します。しかし**計算に使うのは全部「更新前の盤」で、元の盤を書き換えません**（第3章の不変データ）。だからどの順番で計算しても結果は同じ＝いっせいに進めたのと同じになる、というわけです。「いっせい更新」は実装の方法ではなく、**「元の盤を書き換えない」という約束**のことなのです。

「順番に進めるとどう変わるか」を1次元で考えると、次のとおりです。3つの生セル `###` を「左から順に更新する」とします。ルールを「近傍2で生存」に単純化すると（1次元なので近傍は左右の2マスだけ。**端のセルは「外側のマスがない」ので近傍が少なくなる**点に注意）:

```text
最初:  ###
左から順に更新:  左端は右隣1つだけ見て… というように、右端のセルが「更新後の左」を見てしまう
いっせい更新:    全員が最初の ### だけを見るので、どの順番で計算しても同じ結果
```

つまり「誰から更新したか」で結果が変わるのが順次更新、**「元の盤だけを見る」ので結果が一意に決まる**のがいっせい更新です。
...
2. スタブは「まだ正しい計算はしないが、**呼び出せる**」仮の定義（例: 常に `empty` を返す `(define (count-neighbors cell live) 0)`）
3. 目的:
