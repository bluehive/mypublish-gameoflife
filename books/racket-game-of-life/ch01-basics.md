...
「定数だって計算してるじゃん？」と思うかもしれません。違いは**「いつ」計算するか**です。定数 `BOARD-PIXEL-WIDTH` は、ファイルを読み込んだときに**一度だけ**計算されて値（450）が決まります。一方、引数ゼロの関数を許す言語では、**呼ぶたびに本体を実行し直します**。同じ値なのに毎回計算するのは無駄ですし、「呼ぶとその場で計算されるもの」と「最初に決めた値」の区別がつかなくなります。だから BSL は「値が決まっているもの」は定数に、「入力で結果が変わるもの」だけを引数付きの関数にする、と最初から決めているのです。

なお、コメントで「これはエラーになる」と示したコードを実際に入力すると、`define: expected at least one argument` のようなエラーが出ます。これは**あなたのミスではなく BSL の設計**によるものです。気にせず、下の「定数にする」か「引数を取る関数にする」のどちらかを使えば大丈夫です。
...
ただし注意: 「先に書く」のは**頭の中で考える順番**の話です。上のコード例は、説明の都合で `define`（本体）を先に、`check-expect`（例）を後に並べています。ファイルの中での並び順は流儀によって違うので、**1.9 のようにテストを末尾にまとめる**こともあります（一覧性が良くなるから）。「考え方としては例が先、ファイルの並びは何でもよい」と理解してください。**先に約束してからコードを書く**——この順番の違いが、この本の書き方の一番大きなポイントです。
