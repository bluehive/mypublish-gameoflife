...
なぜそんな制限があるのでしょう？ それは BSL が「**関数＝入力を受け取って結果を返す計算**」という考え方を最初からはっきり教えるための言語だからです。引数ゼロの関数を許すと、「毎回計算し直す手続き」と「一度決めた名前付きの値（定数）」の区別がぼやけてしまいます。他の言語が引数ゼロの関数を許すのは便利さ優先。BSL は**考え方の型**を先に身につけるための教育用の制限、と覚えておくと納得しやすいです。
...
ここで大切なのは、`check-expect` が「あとから確認するテスト」ではなく「**例を先に書く**道具」だという点です。ふつうのテストは実装ができたあとに「動くか確かめる」ものですが、この本の `check-expect` は**本体を書く前に**「入力がこうなら出力はこう」を仕様として固定するためのものです。先に例があると「何を作るか」がブレず、Run したときに機械が照合してくれるので、あとから直すときのテストにもなります。**先に約束してからコードを書く**——この順番の違いが、この本の書き方の一番大きなポイントです。
