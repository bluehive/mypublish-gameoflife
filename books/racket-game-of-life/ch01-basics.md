...
なぜそんな制限があるのでしょう？ それは BSL が「**関数＝入力を受け取って結果を返す計算**」という考え方を最初からはっきり教えるための言語だからです。引数ゼロの関数を許すと、「毎回計算し直す手続き」と「一度決めた名前付きの値（定数）」の区別がぼやけてしまいます。

「定数だって計算してるじゃん？」と思うかもしれません。違いは**「いつ・何回」計算するか**です。**結論を先に言うと: 定数も計算します。違いは「1回 vs 呼ぶたび」**です。定数 `BOARD-PIXEL-WIDTH` は、**DrRacket で Run を押した瞬間に**右から左へ評価され、名前 `BOARD-PIXEL-WIDTH` に値 450 が貼り付きます（以後は 450 として扱われる）。一方、引数ゼロの関数を許す言語では、**呼ぶたびに本体を実行し直します**。同じ値なのに毎回計算するのは無駄ですし、「呼ぶとその場で計算されるもの」と「最初に決めた値」の区別がつかなくなります。だから BSL は「値が決まっているもの」は定数に、「入力で結果が変わるもの」だけを引数付きの関数にする、と最初から決めているのです。

なお、コメントで「これはエラーになる」と示したコードを実際に入力すると、`define: expected at least one argument` のようなエラーが出ます。これは**あなたのミスではなく BSL の設計**によるものです。エラーが出ても慌てず、下の「定数にする」か「引数を取る関数にする」のどちらかを使えば大丈夫です。
...
「値が決まっているもの」は定数に、「入力で結果が変わるもの」だけを引数付きの関数にする——これが BSL 流の整理の仕方です。ライフ盤のピクセル幅は `WIDTH` と `CELL-SIZE` から一意に決まるので、本章では定数で十分です。
...
なぜ `else` を最後に置くのでしょう？ それは `cond` が「上から順に試す」からです。もし `[else …]` を先頭に置くと、いつもそこに入ってしまい、下の条件は一度も試されません（書いても意味がなくなります）。

Scratch に置き換えるなら、`cond` は「**でなければ**の中に**もし〜なら**を入れ子にしたもの」に相当します。Scratch の「もし〜なら」ブロックをただ縦に並べただけだと、条件は**全部**チェックされます（最初に当たったところで止まらない）。「上から順に試して、最初に当たったところで止まる」のが `cond` の動きです。具体的な場合を上に並べ、最後に逃げ道の `else`——これが `cond` の基本形です。

**動きを3語で固定**しておくと迷いません: **「条件式は上から順。真の枝の中身だけ実行。残りの枝はスキップ」**。ここで「スキップ」とは「その枝の中身（本体）を実行しない」という意味です。条件式の評価と本体の実行は別物で、真になった枝の本体だけが実行され、それ以降の条件は見ません。
...
ここで大切なのは、`check-expect` が「あとから確認するテスト」ではなく「**例を先に書く**道具」だという点です。ふつうのテストは実装ができたあとに「動くか確かめる」ものですが、この本の `check-expect` は**本体を書く前に**「入力がこうなら出力はこう」を仕様として固定するためのものです。

ただし注意: 「先に書く」のは**頭の中で考える順番**の話です。この本での作業手順を1つに固定します:

1. **ノートやコメントに、まず例を書く** — 「入力がこうなら出力はこう」を2つ以上
2. **次に本体（`define`）を書く** — 例に合うように実装する
3. **`check-expect` はファイルの末尾にまとめる** — 一覧性が良くなる（この本の流儀。1.9 も同じ）

上のコード例は説明の都合で `define` を先に並べていますが、**考える順番は「例 → 本体」**です。「考え方としては例が先、ファイルの並びはこの本ではテスト末尾」と理解してください。**先に約束してからコードを書く**——この順番の違いが、この本の書き方の一番大きなポイントです。
...
```racket
;; このコードを CLI で実行するには、先頭に次の1行が必要です:
;; (require test-engine/racket-tests)
;; （DrRacket で開いて Run する場合は不要。テスト結果は自動表示されます）
...
序章 0.7 で「予想」した5項目の答え合わせです。それぞれ1文で説明できるか確認しましょう。

- [ ] 式（前置記法）を1文で説明できる（例: 演算子を括弧の先頭に置き、引数を並べて計算するコード）
- [ ] `define` で定数（名前付きの値）を作れる（例: `(define WIDTH 30)`。1.3 参照）
- [ ] `cond` で条件分岐を書ける（例: 上から順に条件を試し、最初に真になった枝だけ実行する。1.4 参照）
- [ ] 関数を1文で説明できる（例: 入力（引数）を受け取って出力（結果）を計算する部品。1.5 参照）
- [ ] `check-expect` でテストを書ける（例: `(check-expect (square-of 8) 64)` のように「式」と「期待する結果」を並べ、自動で確かめる。1.9 参照）

すべてにチェックが付いたら、いよいよ第2章（再帰と HtDP テンプレート）へ進みましょう。
