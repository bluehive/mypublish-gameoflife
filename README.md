...
- **序章** — なぜRacketか——ゲームで学ぶ関数型  
  - 原稿: [intro.md](books/racket-game-of-life/intro.md)  
  - コード: —  
  - 状態: howtocode 準拠ドラフト + BSL
- **第1章** — Racketの基礎——式と関数  
  - 原稿: [ch01-basics.md](books/racket-game-of-life/ch01-basics.md)  
  - コード: [ch01-basics.rkt](code/ch01-basics.rkt)  
  - 状態: cheatsheet 準拠 + BSL
- **第2章** — 再帰——Racketの核心  
  - 原稿: [ch02-recursion.md](books/racket-game-of-life/ch02-recursion.md)  
  - コード: [ch02-recursion.rkt](code/ch02-recursion.rkt)  
  - 状態: テンプレート駆動 + BSL
- **第3章** — データ構造——グリッドを表現する  
  - 原稿: [ch03-grid.md](books/racket-game-of-life/ch03-grid.md)  
  - コード: [ch03-grid.rkt](code/ch03-grid.rkt)  
  - 状態: ドラフト + BSL（#3）
- **第4章** — ライフゲームのルールとセルオートマトン  
  - 原稿: [ch04-life-rules.md](books/racket-game-of-life/ch04-life-rules.md)  
  - コード: [ch04-life-rules.rkt](code/ch04-life-rules.rkt)  
  - 状態: ドラフト + BSL（#3）
- **第5章** — 描画と対話——盤面を見る  
  - 原稿: [ch05-display.md](books/racket-game-of-life/ch05-display.md)  
  - コード: [ch05-display.rkt](code/ch05-display.rkt)（本線 BSL）、[ch05-display-isl.rkt](code/ch05-display-isl.rkt)（発展 ISL+）  
  - 状態: 骨格 + big-bang / ISL+ 升目表示（#3）
- **付録 A** — 付属ソース一覧  
  - 原稿: [appendix-a-source-code.md](books/racket-game-of-life/appendix-a-source-code.md)  
  - コード: [code/](https://github.com/bluehive/mypublish-gameoflife/tree/main/code)  
  - 状態: 独立 md（#7）
- **付録 D** — 環境構築  
  - 原稿: [appendix-d-environment.md](books/racket-game-of-life/appendix-d-environment.md)  
  - コード: —  
  - 状態: 独立 md（#3）
- **付録 F** — デバッグ入門  
...
付録 **A–G** — 下記。**A / D / F は独立 md**（B/C/E/G は予定またはメモ）

## 付録（予定）

本編外の参照・手順・チートシート。

- **A** 付属ソースコード一覧（ガイド）— **[appendix-a-source-code.md](books/racket-game-of-life/appendix-a-source-code.md)**  
  正本: [code/ on main](https://github.com/bluehive/mypublish-gameoflife/tree/main/code)（各 `.rkt` の URL と役割）。参考: [bluehive/my-racket](https://github.com/bluehive/my-racket)
- **B** `kyapon-knk-racket.org` 問題50問との対応表
- **C** Racket コマンドライン入門（`racket`, `raco`）
- **D** 環境構築（Windows 11 / DrRacket）— **[appendix-d-environment.md](books/racket-game-of-life/appendix-d-environment.md)**（Issue #3）
...
本文の正本は [appendix-f-debug.md](books/racket-game-of-life/appendix-f-debug.md)。概要のみ:

- **F.1 プリントデバッグ** — 途中の値を見る。BSL 本線は相互作用ウィンドウと `check-expect` が中心。Intermediate 以降の `printf` 例は付録本文
- **F.2**（発展）— `#lang debug` 等。[AlexKnauth/debug](https://github.com/AlexKnauth/debug)
- **F.3 REPL でのデバッグ** — DrRacket 相互作用ウィンドウで小さな式を試す
- **F.4 スタックトレース** — DrRacket の呼び出し履歴（詳細は付録F）
- **F.5**（発展）— `debug-repl`、ログ、リモート等

### 関連 Issue（章・付録の取り込み先）

- [本repo#1](https://github.com/bluehive/mypublish-gameoflife/issues/1)  
  - 取り込み先: 履歴  
  - 内容: 当初 ASL 実験 → **BSL に転換**（ユーザー 0723）
- [#28](https://github.com/bluehive/my-grok-task-2026/issues/28)  
  - 取り込み先: 第1–2章・付録D  
  - 内容: HtDP Beginner / デザインレシピ
- [#29](https://github.com/bluehive/my-grok-task-2026/issues/29)  
  - 取り込み先: 第4–5章  
  - 内容: Game of Life 実装・テスト
...
- `code/*.rkt` … 章付属コード（`#lang htdp/bsl` + `check-expect`。発展は ISL+ もあり）
...
初回: 本は `published: true` / **chapters は序章 (`intro`) のみ** から開始（#27 Q3）。現状の公開章は `config.yaml` を確認。
