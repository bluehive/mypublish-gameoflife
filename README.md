...
| 付録 A | 付属ソース一覧 | [appendix-a-source-code.md](books/racket-game-of-life/appendix-a-source-code.md) | [code/](https://github.com/bluehive/mypublish-gameoflife/tree/main/code) | 独立 md（#7） |
| 付録 F | デバッグ入門 | [appendix-f-debug.md](books/racket-game-of-life/appendix-f-debug.md) | — | F.1/3/4（#7） |
...
付録 **A–G** — 下記。**A / D / F は独立 md**（B/C/E/G は予定または README メモ）
...
- **F** デバッグ方法（入門）— **[appendix-f-debug.md](books/racket-game-of-life/appendix-f-debug.md)**（Issue #7: **F.1 / F.3 / F.4**）  
...
- **F.1 プリントデバッグ** — 途中の値を見る。BSL 本線は相互作用ウィンドウと `check-expect` が中心。Intermediate 以降の `printf` 例は付録本文・[debugging-in-racket](https://www.brinckerhoff.org/clements/2252-csc430/Files/debugging-in-racket.html)
- **F.2**（今回の付録本文では扱わない・発展）— `#lang debug` 等。[AlexKnauth/debug](https://github.com/AlexKnauth/debug)
- **F.3 REPL でのデバッグ** — DrRacket 相互作用ウィンドウで小さな式を試す。[CL debugging cookbook](https://lispcookbook.github.io/cl-cookbook/debugging.html)（発想の参考）
- **F.4 trace** — 呼び出しの記録。BSL では第2章の式トレース＋`check-expect` を推奨
- **F.5**（今回の付録本文では扱わない）— `debug-repl`、ログ、リモート等
